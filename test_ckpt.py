...
print(f"Dense (full teacher)   \t\t: ppl={dense_full['ppl']:.3f}\ttokens={dense_full['tokens']}")
...
total_time = time.time() - start_time
print(
    f"\nRandom (matched keep)  \t\t: ppl={rand_matched['ppl']:.3f}  "
    f"keep_all={rand_matched['avg_keep_all']:.3f}  "
    f"prune_keep={rand_matched['avg_prune_keep']:.3f}  "
    f"quant_ratio={16*rand_matched['avg_quant_ratio']:.3f}\t"
    f"tokens={rand_matched['tokens_effective']}/{rand_matched['tokens']}\n"
)
...
print(
    ...
    f"keep_all={rand_matched['avg_keep_all']:.3f}  "
    f"keep_eff={rand_matched['avg_keep_effective']:.3f}\t"
    f"tokens={rand_matched['tokens_effective']}/{rand_matched['tokens']}"
)
print(
...
print(
    ...
    f"high_k={rand_matched['mix_hi_k']:.2f}, p_high={rand_matched['mix_p_hi']:.3f}"
)
...
csv_path = "ckpt_perplexities.csv"
...
row = {
    ...
    "dense_ppl": float(dense_full["ppl"]),
    "sft_teacher_ppl": float(teacher_matched["ppl"]),
    "policy_ppl": float(greedy["ppl"]),
    "random_ppl": float(rand_matched["ppl"]),
    ...
    "random_keep_all": float(rand_matched["avg_keep_all"]),
...
fieldnames = [
    "ckpt_dir", "mode", "dataset_name", "sparsity_bias",
    ...
    "sparsity_levels_kappa_order", "policy_action_probs_kappa_order",
]
