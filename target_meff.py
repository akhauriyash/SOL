...
# Ensure SDPA is enabled in the "mem_efficient" mode only
...
parser.add_argument(
    "--dataset_name",
    type=str,
    choices=["wikitext", "allenai/c4"],
    default=None,
)

# NEW: explicit targets instead of biases
parser.add_argument(
    "--tgt_keep",
    type=float,
    default=None,
    help="Desired effective keep ratio (tokens_effective / tokens). "
         "If None, baseline matches the policy's avg_keep_effective."
)
parser.add_argument(
    "--tgt_prune_keep",
    type=float,
    default=None,
    help="Desired structured-prune keep ratio. "
         "If None, baseline matches the policy's avg_prune_keep."
)
parser.add_argument(
    "--tgt_quant_ratio",
    type=float,
    default=None,
    help="Desired quantization ratio in [0,1]. "
         "If None, baseline matches the policy's avg_quant_ratio."
)
...
# Evaluation budgets for the RL policy:
# If CLI targets are passed, use them; otherwise, let the evaluator fall back
# to cfg.{C_target_token, C_target_prune, C_target_quant_bits}.
...
eval_C_qbits = None
if args.tgt_quant_ratio is not None:
    eval_C_qbits = 16.0 * float(args.tgt_quant_ratio)  # bits = ratio * 16
...
# The fixed baseline is evaluated at what the policy actually achieved,
# not what it was *targeted* to achieve.
...
# -------------------- Fixed matched baseline at explicit targets -------------------- #
...
row = {
    ...
    # Explicit targets used for the fixed baseline
