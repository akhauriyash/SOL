...
CSV_PATH = "multi_efficiency_plot.csv"
OUT_DELTA_EDC = "multi_efficiency_plot_edc.pdf"
...
if not rows:
    ...
    plt.savefig("multi_efficiency_plot_hist.pdf", bbox_inches="tight")
    print("Saved multi_efficiency_plot_hist.pdf")
    edrate = 1.0
    ...
    # print min max of each bias
    print("\nBias ranges in full dataframe:")
    for bias_col in [sparsity_col, "prune_bias", "quant_bias"]:
        vals = pd.to_numeric(df[bias_col], errors="coerce")
        print(f"  {bias_col}: min={np.nanmin(vals):.6f}, max={np.nanmax(vals):.6f}")
